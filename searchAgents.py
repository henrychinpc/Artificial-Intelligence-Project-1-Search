        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if nextState in self.corners:
                    if nextState not in nextVisitedCorners:
...
def cornersHeuristic(state, problem):
    ...
    for c in corners:
        if c not in visitedCorners:
            unvisitedCorners.append(c)

    while unvisitedCorners:
        distanceArray = []
        for c in range(0, len(unvisitedCorners)):
            corner = unvisitedCorners[c]
            distance = util.manhattanDistance(position, corner)
            distanceArray.append(distance)
        minDistance = min(distanceArray)
        minDis = distanceArray.index(minDistance)
        position = unvisitedCorners[minDis]
...
def foodHeuristic(state, problem):
    ...
    for foodPos in foodGrid:
        heuristicArray = []
        heuristic = mazeDistance(foodPos, position, problem.startingGameState)
        heuristicArray.append(heuristic)
        cost = max(heuristicArray)
